...
int main() {
    ...
    int column;
    int answer = 0;
    sudokuFile = fopen("sudoku.txt", "r");

    if (sudokuFile) {
        for (number = 0; number < 50; number++) {
            fscanfResult = fscanf(sudokuFile, "%s %d ", grid, &gridNumber);

            snprintf(assetFileName, sizeof(char) * 16, "puzzle_%02d.txt", gridNumber);

            assetFile = fopen(assetFileName, "w");
            fprintf(assetFile, "Puzzle %d\n", gridNumber);

            for (row = 0; row < 9; row++) {
                fscanfResult = fscanf(sudokuFile, "%d ", &digits);

                fprintf(assetFile, "%09d\n", digits);

                for (column = 0; column < 9; column++) {
                    puzzle[row][8 - column] = digits % 10;
                    digits /= 10;
                }
            }
